...
export default function example() {
    ...
    const imagePanels = [];
    let imagePanel;
    for (let i =0 ; i< spherePositionArray.length; i += 3)  // xyz 점 3개니까
    {
        imagePanel = new ImagePanel({
            textureLoader,
            scene,
            geometry: planeGeometry,
            imageSrc: `/images/0${Math.ceil(Math.random() * 5)}.jpg`,  // ceil 올림
    ...
    function setShape(e){
        let array;
        // console.log(e.target);  // 이렇게 누른 버튼 먼지 알 수 있음
    ...
    randomBtn.dataset.type = 'random';  // button - data-type= "random" 으로 들어간다.
    ...
    sphereBtn.dataset.type = 'sphere';   // button - data-type= "sphere" 으로 들어간다.
