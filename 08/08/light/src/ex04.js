...
// ----- 주제: SpotLight 특정 지지점만 빛을 쫘왁 깔아주는것
...
export default function example() {
    ...
    // renderer.shadowMap.type = THREE.BasicShadowMap;   // 먼가 픽셀적이게 성능 좋음
	...
	const light = new THREE.SpotLight('white',1, 100, MathUtils.degToRad(30));  // spotlight
	light.position.y=3;
	scene.add(light);
	
	const lightHelper = new THREE.SpotLightHelper(light);  // light의 위치가 어딘지 시각적으로 보인다.
	scene.add(lightHelper);

    // 그림자 설정
    light.castShadow = true;
    light.shadow.mapSize.width = 2048;   // 그림자 해상도를 높여서 고퀄리티로 만듬  기본값은 512
    light.shadow.mapSize.height = 2048;
    // light.shadow.radius = 15;   // 그림자를 뿌옇게 THREE.PCFShadowMap에만 적용 가능
   
    light.shadow.camera.near = 1;   // 그림자의 범위를 정해줘서 쓸데없는 공간에 그림자를 넣지않게 성능향상
